    ...
    if (excel != null) {
      for (var table in excel.tables.keys) {
        ...
        for (var row in excel.tables[table]!.rows) {
          ...
          if (row[0] != null) {
            Data? dataA = row[0];
            quizModel.questionNormal = dataA!.value;
          }
          //Đề bài(furigana)
          if (row[1] != null) {
            Data? dataB = row[1];
            quizModel.questionFurigana = convertTextToRuby(dataB!.value);
          }
          if (row[2] != null) {
            Data dataC = row[2]; //Đề bài(translate)
            quizModel.questionTranslate = dataC.value;
          }
